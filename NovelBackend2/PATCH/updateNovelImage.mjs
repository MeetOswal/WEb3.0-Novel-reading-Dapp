...
app.patch("/:novelid", upload.single("image"), async(req, res) => {

    try {
    const token = jwt.verify(req.cookies.JWT, "MeetOswal");
    const novel = await NOVEL.findOne({_id : req.params.novelid});
    const user = await User.findOne({_id : token._id});
    if(user.username !== novel.novelCreator){
        return res.status(200).send({status : "failure"})
    }

    let url = await UploadToIPFS(filename, novel._id);
    await NOVEL.updateOne({_id : token._id},{
        $set : {
            novelThumbnail : url
        }
    })
